...
public class Reader {
	...
	/**
	 * 
	 * @param args 0 is the host name of the master server <String> <br>
	...
	public static void main(String[] args)
		...
		if (args.length < 3) {
			System.out.println(
					"start command has to be <host name> <port number> <file name>");
			System.exit(-1);
		}
		String host = args[0];
		...
		String path = System.getProperty("user.dir") + "/Readers/" + fileName
				+ ".txt";
		BufferedWriter fileWriter = new BufferedWriter(new FileWriter(path));

		Registry masterRegistry = LocateRegistry.getRegistry(host, port);
		MasterServerClientInterface masterStub = (MasterServerClientInterface) masterRegistry
				.lookup("masterAPI");
		...
		try {
			...
			Registry ReplicaRegistry = LocateRegistry.getRegistry(
					primaryReplica.getHost(), primaryReplica.getPort());
